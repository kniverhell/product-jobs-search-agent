...
Write the report to `reports/YYYY-MM-DD-company-slug.md` with these seven blocks. Then update the tracker to `Evaluated` with the score and archetype.
...
## B. Fit assessment
Score all five dimensions with a sentence each, then the holistic 1.0–5.0. Name the archetype. Then state the **location tier and modifier** from `config.md`, and the final score — showing both numbers. If the modifier is what pushes it below the Apply threshold, say that explicitly; it's a different decision from a role that's simply weak.
...
## F. Interview stories
Which three stories from `04-story-bank.md` this role would draw on. If the role needs a story that doesn't exist yet, **draft the skeleton and append it to the story bank.** The bank grows on every evaluation — that's the compounding part.
